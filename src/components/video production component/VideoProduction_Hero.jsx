...
import video_production_hero_image from '../../assets/video_production_hero_image.svg'
...
const VideoProduction_Hero = () => {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(
      ".video-production-animated-item",
      {
        opacity: 0,
        y: 50,
      },
  ...
  return (
    <section className="px-5 py-5 sm:px-10 sm:py-10  md:px-20  md:py-20  ">
      <div className="flex flex-col-reverse gap-5 md:flex-row items-center md:justify-between">
        <div className="flex video-production-text flex-col basis-full md:basis-[60%] text-center md:text-left">
        ...
        <img src={video_production_hero_image} alt="" className='video-production-circle' />
      </div>
      ...
      <div className="flex justify-center items-center flex-wrap   my-16">
        {[
          ...
          <p
            key={index}
            className="px-4 py-4 video-production-animated-item hover:text-violet-800 hover:border-violet-800 cursor-pointer text-gray-600 border border-gray-300 rounded-3xl text-xl"
          >
            {text}
          </p>
        ))}
